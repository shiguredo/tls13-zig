...
test "client test with RFC8448" {
    ...
    const handshake = try msg.Handshake.decode(io.fixedBufferStream(&server_hello_bytes).reader(), std.testing.allocator, null);
    ...
    try ks.generateResumptionMasterSecret(c_record_finished_bytes[0..c_finished_write_len]);
