...
pub const TLSClient = struct {
    ...
    pub fn connect(self: *Self, reader: anytype, writer: anytype) !void {
        ...
        while (self.state != .CONNECTED) {
            ...
            if (self.state == .WAIT_SH) {
                ...
                const plain_record = try Self.Protector.decrypt(recv_record, nonce.slice()[0..12].*, self.ks.secret.s_hs_key.slice()[0..16].*, self.allocator);
    ...
    pub fn send(self: *Self, b: []const u8, writer: anytype) !usize {
        ...
        const write_enc = try Self.Protector.encryptFromPlainBytes(b, .application_data, nonce.slice()[0..12].*, self.ks.secret.c_ap_key.slice()[0..16].*, self.allocator);
    ...
    pub fn recv(self: *Self, b: []u8, reader: anytype) !usize {
        ...
        while (!ap_recv) {
            ...
            const nonce = try self.ks.generateNonce(self.ks.secret.s_ap_iv.slice(), self.recv_count);
            const plain_record = try Self.Protector.decrypt(recv_record, nonce.slice()[0..12].*, self.ks.secret.s_ap_key.slice()[0..16].*, self.allocator);
            defer plain_record.deinit();
            self.recv_count += 1;
    ...
    pub fn close(self: *Self, reader: anytype, writer: anytype) !void {
        ...
        const c_alert_record = try Protector.encryptFromPlainBytes(&c_alert_data, .alert, nonce.slice()[0..12].*, self.ks.secret.c_ap_key.slice()[0..16].*, self.allocator);
        ...
        while (!close_recv) {
            ...
            nonce = try self.ks.generateNonce(self.ks.secret.s_ap_iv.slice(), self.recv_count);
            const plain_record = try Self.Protector.decrypt(recv_record, nonce.slice()[0..12].*, self.ks.secret.s_ap_key.slice()[0..16].*, self.allocator);
            defer plain_record.deinit();
            self.recv_count += 1;
    ...
    fn handleHandshakeInnerPlaintext(self: *Self, t: record.TLSInnerPlainText, writer: anytype) !void {
        ...
        while ((try stream.getPos()) != (try stream.getEndPos())) {
            ...
            if (self.state == .SEND_FINISHED) {
                ...
                const c_record_finished = try Protector.encrypt(c_finished_inner, nonce.slice()[0..12].*, self.ks.secret.c_hs_key.slice()[0..16].*, self.allocator);
...
test "client test with RFC8448" {
    ...
    var nonce = try ks.generateNonce(ks.secret.s_hs_iv.slice(), 0);
    const pt_misc = try Protector.decryptFromCipherBytes(&enc_data, nonce.slice()[0..12].*, ks.secret.s_hs_key.slice()[0..16].*, std.testing.allocator);
    ...
    const c_record_finished = try Protector.encrypt(c_finished_inner, nonce.slice()[0..12].*, ks.secret.c_hs_key.slice()[0..16].*, std.testing.allocator);
    defer c_record_finished.deinit();
    ...
    nonce = try ks.generateNonce(ks.secret.s_ap_iv.slice(), 0);
    const pt_s_ticket = try Protector.decryptFromCipherBytes(&s_ticket_enc, nonce.slice()[0..12].*, ks.secret.s_ap_key.slice()[0..16].*, std.testing.allocator);
    ...
    const c_app_record = try Protector.encryptFromPlainBytes(&c_app_data, .application_data, nonce.slice()[0..12].*, ks.secret.c_ap_key.slice()[0..16].*, std.testing.allocator);
    ...
    nonce = try ks.generateNonce(ks.secret.s_ap_iv.slice(), 1);
    ...
    const pt_recv_ap = try Protector.decryptFromCipherBytes(&s_app_record_enc, nonce.slice()[0..12].*, ks.secret.s_ap_key.slice()[0..16].*, std.testing.allocator);
    ...
    const c_alert_record = try Protector.encryptFromPlainBytes(&c_alert_data, .alert, nonce.slice()[0..12].*, ks.secret.c_ap_key.slice()[0..16].*, std.testing.allocator);
    defer c_alert_record.deinit();
    ...
    nonce = try ks.generateNonce(ks.secret.s_ap_iv.slice(), 2);
    const pt_recv_alert = try Protector.decryptFromCipherBytes(&s_alert_enc, nonce.slice()[0..12].*, ks.secret.s_ap_key.slice()[0..16].*, std.testing.allocator);
