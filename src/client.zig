const std = @import("std");
const io = std.io;
const dh = std.crypto.dh;
const hmac = std.crypto.auth.hmac;
const hkdf = std.crypto.kdf.hkdf;
const expect = std.testing.expect;
const expectError = std.testing.expectError;

const msg = @import("msg.zig");
const key = @import("key.zig");

fn AesGcmTLS(comptime AesGcm: anytype) type {

    return struct {
        pub const tag_length = AesGcm.tag_length;
        pub const nonce_length = AesGcm.nonce_length;
        pub const key_length = AesGcm.key_length;

        const Self = @This();
        pub fn encrypt(c: []u8, m: []const u8, n: [nonce_length]u8, k: [key_length]u8) void {
            AesGcm.encrypt(c[5..(c.len - Self.tag_length)], c[(c.len - Self.tag_length)..][0..Self.tag_length], m, c[0..5], n, k);
        }

        pub fn decrypt(m: []u8, c: []const u8, n: [nonce_length]u8, k: [key_length]u8) std.crypto.errors.AuthenticationError!void {
            try AesGcm.decrypt(m, c[5..c.len - 16], c[c.len - 16..][0..Self.tag_length].*, c[0..5], n, k);
        }
    };
}

// this test rlies on RFC8446 Appendix A.1. Client(page 120)
test "client test with RFC8448" {
    // STATE = START

    const client_privkey = [_]u8{ 0x49, 0xaf, 0x42, 0xba, 0x7f, 0x79, 0x94, 0x85, 0x2d, 0x71, 0x3e, 0xf2, 0x78, 0x4b, 0xcb, 0xca, 0xa7, 0x91, 0x1d, 0xe2, 0x6a, 0xdc, 0x56, 0x42, 0xcb, 0x63, 0x45, 0x40, 0xe7, 0xea, 0x50, 0x05 };
    const client_pubkey_ans = [_]u8{ 0x99, 0x38, 0x1d, 0xe5, 0x60, 0xe4, 0xbd, 0x43, 0xd2, 0x3d, 0x8e, 0x43, 0x5a, 0x7d, 0xba, 0xfe, 0xb3, 0xc0, 0x6e, 0x51, 0xc1, 0x3c, 0xae, 0x4d, 0x54, 0x13, 0x69, 0x1e, 0x52, 0x9a, 0xaf, 0x2c };
    const client_pubkey = try dh.X25519.recoverPublicKey(client_privkey);
    try expect(std.mem.eql(u8, &client_pubkey, &client_pubkey_ans));

    const client_hello_bytes = [_]u8{};

    // STATE = WAIT_SH

    const server_hello_bytes = [_]u8{0x02, 0x00, 0x00, 0x56, 0x03, 0x03, 0xa6, 0xaf, 0x06, 0xa4, 0x12, 0x18, 0x60, 0xdc, 0x5e, 0x6e, 0x60, 0x24, 0x9c, 0xd3, 0x4c, 0x95, 0x93, 0x0c, 0x8a, 0xc5, 0xcb, 0x14, 0x34, 0xda, 0xc1, 0x55, 0x77, 0x2e, 0xd3, 0xe2, 0x69, 0x28, 0x00, 0x13, 0x01, 0x00, 0x00, 0x2e, 0x00, 0x33, 0x00, 0x24, 0x00, 0x1d, 0x00, 0x20, 0xc9, 0x82, 0x88, 0x76, 0x11, 0x20, 0x95, 0xfe, 0x66, 0x76, 0x2b, 0xdb, 0xf7, 0xc6, 0x72, 0xe1, 0x56, 0xd6, 0xcc, 0x25, 0x3b, 0x83, 0x3d, 0xf1, 0xdd, 0x69, 0xb1, 0xb0, 0x4e, 0x75, 0x1f, 0x0f, 0x00, 0x2b, 0x00, 0x02, 0x03, 0x04};
    var readStream = io.fixedBufferStream(&server_hello_bytes);
    const handshake = try msg.Handshake.decode(readStream.reader(), std.testing.allocator, null);
    try expect(handshake == .server_hello);

    const server_hello = handshake.server_hello;
    defer server_hello.deinit();

    try expect(server_hello.cipher_suite == .TLS_AES_128_GCM_SHA256);
    const key_share = (try msg.getExtension(server_hello.extensions, .key_share)).key_share;
    try expect(key_share.entries.items.len == 1);
    try expect(key_share.entries.items[0] == .x25519);

    const server_pubkey = key_share.entries.items[0].x25519.key_exchange;

    const Aes128Gcm = std.crypto.aead.aes_gcm.Aes128Gcm;
    const Aes128GcmTLS = AesGcmTLS(std.crypto.aead.aes_gcm.Aes128Gcm);
    const Sha256 = std.crypto.hash.sha2.Sha256;

    const early_secret_ans = [_]u8{ 0x33, 0xad, 0x0a, 0x1c, 0x60, 0x7e, 0xc0, 0x3b, 0x09, 0xe6, 0xcd, 0x98, 0x93, 0x68, 0x0c, 0xe2, 0x10, 0xad, 0xf3, 0x00, 0xaa, 0x1f, 0x26, 0x60, 0xe1, 0xb2, 0x2e, 0x10, 0xf1, 0x70, 0xf9, 0x2a };
    const early_secret = key.deriveEarlySecret(Sha256, &([_]u8{0} ** 32));

    try expect(std.mem.eql(u8, &early_secret, &early_secret_ans));
    
    const handshake_derived_secret_ans = [_]u8{ 0x6f, 0x26, 0x15, 0xa1, 0x08, 0xc7, 0x02, 0xc5, 0x67, 0x8f, 0x54, 0xfc, 0x9d, 0xba, 0xb6, 0x97, 0x16, 0xc0, 0x76, 0x18, 0x9c, 0x48, 0x25, 0x0c, 0xeb, 0xea, 0xc3, 0x57, 0x6c, 0x36, 0x11, 0xba };
    const handshake_derived_secret = try key.deriveSecret(Sha256, early_secret, "derived", "");
    try expect(std.mem.eql(u8, &handshake_derived_secret, &handshake_derived_secret_ans));

    const dhe_shared_key_ans = [_]u8{ 0x8b, 0xd4, 0x05, 0x4f, 0xb5, 0x5b, 0x9d, 0x63, 0xfd, 0xfb, 0xac, 0xf9, 0xf0, 0x4b, 0x9f, 0x0d, 0x35, 0xe6, 0xd6, 0x3f, 0x53, 0x75, 0x63, 0xef, 0xd4, 0x62, 0x72, 0x90, 0x0f, 0x89, 0x49, 0x2d };
    const dhe_shared_key = try dh.X25519.scalarmult(client_privkey, server_pubkey);
    try expect(std.mem.eql(u8, &dhe_shared_key, &dhe_shared_key_ans));

    const handshake_secret_ans = [_]u8{ 0x1d, 0xc8, 0x26, 0xe9, 0x36, 0x06, 0xaa, 0x6f, 0xdc, 0x0a, 0xad, 0xc1, 0x2f, 0x74, 0x1b, 0x01, 0x04, 0x6a, 0xa6, 0xb9, 0x9f, 0x69, 0x1e, 0xd2, 0x21, 0xa9, 0xf0, 0xca, 0x04, 0x3f, 0xbe, 0xac };
    const handshake_secret = hkdf.Hkdf(hmac.Hmac(Sha256)).extract(&handshake_derived_secret, &dhe_shared_key);
    try expect(std.mem.eql(u8, &handshake_secret, &handshake_secret_ans));

    var hs_msg = client_hello_bytes ++ server_hello_bytes;
    const c_hs_traffic_secret_ans = [_]u8{ 0xb3, 0xed, 0xdb, 0x12, 0x6e, 0x06, 0x7f, 0x35, 0xa7, 0x80, 0xb3, 0xab, 0xf4, 0x5e, 0x2d, 0x8f, 0x3b, 0x1a, 0x95, 0x07, 0x38, 0xf5, 0x2e, 0x96, 0x00, 0x74, 0x6a, 0x0e, 0x27, 0xa5, 0x5a, 0x21 };
    const c_hs_traffic_secret = try key.deriveSecret(Sha256, handshake_secret, "c hs traffic", &hs_msg);
    try expect(std.mem.eql(u8, &c_hs_traffic_secret, &c_hs_traffic_secret_ans));

    const s_hs_traffic_secret_ans = [_]u8{ 0xb6, 0x7b, 0x7d, 0x69, 0x0c, 0xc1, 0x6c, 0x4e, 0x75, 0xe5, 0x42, 0x13, 0xcb, 0x2d, 0x37, 0xb4, 0xe9, 0xc9, 0x12, 0xbc, 0xde, 0xd9, 0x10, 0x5d, 0x42, 0xbe, 0xfd, 0x59, 0xd3, 0x91, 0xad, 0x38 };
    const s_hs_traffic_secret = try key.deriveSecret(Sha256, handshake_secret, "s hs traffic", &hs_msg);
    try expect(std.mem.eql(u8, &s_hs_traffic_secret, &s_hs_traffic_secret_ans));

    const master_derived_secret_ans = [_]u8{ 0x43, 0xde, 0x77, 0xe0, 0xc7, 0x77, 0x13, 0x85, 0x9a, 0x94, 0x4d, 0xb9, 0xdb, 0x25, 0x90, 0xb5, 0x31, 0x90, 0xa6, 0x5b, 0x3e, 0xe2, 0xe4, 0xf1, 0x2d, 0xd7, 0xa0, 0xbb, 0x7c, 0xe2, 0x54, 0xb4 };
    const master_derived_secret = try key.deriveSecret(Sha256, handshake_secret, "derived", "");
    try expect(std.mem.eql(u8, &master_derived_secret, &master_derived_secret_ans));

    const master_secret_ans = [_]u8{ 0x18, 0xdf, 0x06, 0x84, 0x3d, 0x13, 0xa0, 0x8b, 0xf2, 0xa4, 0x49, 0x84, 0x4c, 0x5f, 0x8a, 0x47, 0x80, 0x01, 0xbc, 0x4d, 0x4c, 0x62, 0x79, 0x84, 0xd5, 0xa4, 0x1d, 0xa8, 0xd0, 0x40, 0x29, 0x19 };
    const master_secret = hkdf.Hkdf(hmac.Hmac(Sha256)).extract(&master_derived_secret, &([_]u8{0} ** 32));
    try expect(std.mem.eql(u8, &master_secret, &master_secret_ans));

    // STATE = WAIT_EE

    const hs_read_key_ans = [_]u8{0x3f, 0xce, 0x51, 0x60, 0x09, 0xc2, 0x17, 0x27, 0xd0, 0xf2, 0xe4, 0xe8, 0x6e, 0xe4, 0x03, 0xbc};
    const hs_read_iv_ans = [_]u8{0x5d, 0x31, 0x3e, 0xb2, 0x67, 0x12, 0x76, 0xee, 0x13, 0x00, 0x0b, 0x30};

    const hs_read_key = try key.hkdfExpandLabel(Sha256, s_hs_traffic_secret, "key", "", Aes128Gcm.key_length);
    try expect(std.mem.eql(u8, &hs_read_key, &hs_read_key_ans));

    const hs_read_iv = try key.hkdfExpandLabel(Sha256, s_hs_traffic_secret, "iv", "", Aes128Gcm.nonce_length);
    try expect(std.mem.eql(u8, &hs_read_iv, &hs_read_iv_ans));

    // RFC8446 Section 5.3 Pre-Record Nonce
    var hs_read_nonce = [_]u8{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
    var i:usize = 0;
    while (i < hs_read_iv.len): (i += 1) {
        hs_read_nonce[i] = hs_read_iv[i] ^ hs_read_nonce[i];
    }

    // RFC 8446 Section 5.2 Record Payload Protection
    const enc_data = [_]u8{};
    var plain_payload:[enc_data.len - 5 - Aes128GcmTLS.tag_length]u8 = undefined;
    try Aes128GcmTLS.decrypt(&plain_payload, &enc_data, hs_read_nonce, hs_read_key);
    
    // decode EncryptedExtensions
    var readStream2 = io.fixedBufferStream(&plain_payload);
    const enc_ext = (try msg.Handshake.decode(readStream2.reader(), std.testing.allocator, null)).encrypted_extensions;
    defer enc_ext.deinit();

    try expect(enc_ext.extensions.items.len == 3);
    try expect(enc_ext.extensions.items[0] == .supported_groups);
    try expect(enc_ext.extensions.items[1] == .record_size_limit);
    try expect(enc_ext.extensions.items[2] == .server_name);

    // STATE = WAIT_CERT_CR

    // decode Certificate
    const cert = (try msg.Handshake.decode(readStream2.reader(), std.testing.allocator, null)).certificate;
    defer cert.deinit();
    try expect(cert.cert_req_ctx.len == 0);
    try expect(cert.cert_list.items.len == 1);
    try expect(cert.cert_list.items[0].cert_data.items.len == 432);
    try expect(cert.cert_list.items[0].extensions.items.len == 0);

    // WAIT_CV

    // decode CertificateVerify
    const cert_verify = (try msg.Handshake.decode(readStream2.reader(), std.testing.allocator, null)).certificate_verify;
    defer cert_verify.deinit();
    try expect(cert_verify.algorithm == .rsa_pss_rsae_sha256);

    // STATE = WAIT_FINISHED

    // decode Finished
    const s_hs_finished = (try msg.Handshake.decode(readStream2.reader(), std.testing.allocator, Sha256)).finished;
    defer s_hs_finished.deinit();

    // check all data was read.
    const content_type = try readStream2.reader().readIntBig(u8);
    try expect(content_type == 0x16);
    try expectError(error.EndOfStream, readStream2.reader().readByte());

    var app_hs_bytes:[client_hello_bytes.len + server_hello_bytes.len + plain_payload.len - 1 + 4 + Sha256.digest_length]u8 = undefined;
    var app_hs_stream = io.fixedBufferStream(&app_hs_bytes);
    try app_hs_stream.writer().writeAll(&client_hello_bytes);
    try app_hs_stream.writer().writeAll(&server_hello_bytes);
    try app_hs_stream.writer().writeAll(plain_payload[0..(plain_payload.len-1)]);

    // validate "finished"
    const s_hs_finished_key_ans = [_]u8{ 0x00, 0x8d, 0x3b, 0x66, 0xf8, 0x16, 0xea, 0x55, 0x9f, 0x96, 0xb5, 0x37, 0xe8, 0x85, 0xc3, 0x1f, 0xc0, 0x68, 0xbf, 0x49, 0x2c, 0x65, 0x2f, 0x01, 0xf2, 0x88, 0xa1, 0xd8, 0xcd, 0xc1, 0x9f, 0xc8 };
    const s_hs_finished_key = try key.hkdfExpandLabel(Sha256, s_hs_traffic_secret, "finished", "", Sha256.digest_length);
    try expect(std.mem.eql(u8, &s_hs_finished_key, &s_hs_finished_key_ans));

    var s_hs_finished_digest: [Sha256.digest_length]u8 = undefined;
    var s_hs_hash: [Sha256.digest_length]u8 = undefined;
    Sha256.hash(app_hs_bytes[0..(try app_hs_stream.getPos()) - 4 - s_hs_finished.length()], &s_hs_hash, .{}); // ignore "finished" record
    hmac.Hmac(Sha256).create(&s_hs_finished_digest, &s_hs_hash, &s_hs_finished_key);
    try expect(std.mem.eql(u8, &s_hs_finished_digest, s_hs_finished.verify_data.slice()));

    const c_ap_traffic_secret_ans = [_]u8{ 0x9e, 0x40, 0x64, 0x6c, 0xe7, 0x9a, 0x7f, 0x9d, 0xc0, 0x5a, 0xf8, 0x88, 0x9b, 0xce, 0x65, 0x52, 0x87, 0x5a, 0xfa, 0x0b, 0x06, 0xdf, 0x00, 0x87, 0xf7, 0x92, 0xeb, 0xb7, 0xc1, 0x75, 0x04, 0xa5 };
    const c_ap_traffic_secret = try key.deriveSecret(Sha256, master_secret, "c ap traffic", app_hs_bytes[0..(try app_hs_stream.getPos())]);
    try expect(std.mem.eql(u8, &c_ap_traffic_secret, &c_ap_traffic_secret_ans));

    const s_ap_traffic_secret_ans = [_]u8{ 0xa1, 0x1a, 0xf9, 0xf0, 0x55, 0x31, 0xf8, 0x56, 0xad, 0x47, 0x11, 0x6b, 0x45, 0xa9, 0x50, 0x32, 0x82, 0x04, 0xb4, 0xf4, 0x4b, 0xfb, 0x6b, 0x3a, 0x4b, 0x4f, 0x1f, 0x3f, 0xcb, 0x63, 0x16, 0x43 };
    const s_ap_traffic_secret = try key.deriveSecret(Sha256, master_secret, "s ap traffic", app_hs_bytes[0..(try app_hs_stream.getPos())]);
    try expect(std.mem.eql(u8, &s_ap_traffic_secret, &s_ap_traffic_secret_ans));

    const hs_write_key_ans = [_]u8{ 0xdb, 0xfa, 0xa6, 0x93, 0xd1, 0x76, 0x2c, 0x5b, 0x66, 0x6a, 0xf5, 0xd9, 0x50, 0x25, 0x8d, 0x01 };
    const hs_write_iv_ans = [_]u8{ 0x5b, 0xd3, 0xc7, 0x1b, 0x83, 0x6e, 0x0b, 0x76, 0xbb, 0x73, 0x26, 0x5f };
    const hs_write_key = try key.hkdfExpandLabel(Sha256, c_hs_traffic_secret, "key", "", Aes128Gcm.key_length);
    try expect(std.mem.eql(u8, &hs_write_key, &hs_write_key_ans));
    const hs_write_iv = try key.hkdfExpandLabel(Sha256, c_hs_traffic_secret, "iv", "", Aes128Gcm.nonce_length);
    try expect(std.mem.eql(u8, &hs_write_iv, &hs_write_iv_ans));
    var hs_write_nonce = [_]u8{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
    i = 0;
    while (i < hs_write_iv.len): (i += 1) {
        hs_write_nonce[i] = hs_write_iv[i] ^ hs_write_nonce[i];
    }

    // Construct client finised message
    const c_hs_finished_key_ans = [_]u8{0xb8, 0x0a, 0xd0, 0x10, 0x15, 0xfb, 0x2f, 0x0b, 0xd6, 0x5f, 0xf7, 0xd4, 0xda, 0x5d, 0x6b, 0xf8, 0x3f, 0x84, 0x82, 0x1d, 0x1f, 0x87, 0xfd, 0xc7, 0xd3, 0xc7, 0x5b, 0x5a, 0x7b, 0x42, 0xd9, 0xc4};
    const c_hs_finished_key = try key.hkdfExpandLabel(Sha256, c_hs_traffic_secret, "finished", "", Sha256.digest_length);
    try expect(std.mem.eql(u8, &c_hs_finished_key, &c_hs_finished_key_ans));

    var c_hs_finished_digest: [Sha256.digest_length]u8 = undefined;
    var c_hs_hash: [Sha256.digest_length]u8 = undefined;
    Sha256.hash(app_hs_bytes[0..(try app_hs_stream.getPos())], &c_hs_hash, .{}); // ignore "finished" record
    hmac.Hmac(Sha256).create(&c_hs_finished_digest, &c_hs_hash, &c_hs_finished_key);

    const c_finished_ans = [_]u8{0x14, 0x0, 0x0, 0x20, 0xa8, 0xec, 0x43, 0x6d, 0x67, 0x76, 0x34, 0xae, 0x52, 0x5a, 0xc1, 0xfc, 0xeb, 0xe1, 0x1a, 0x03, 0x9e, 0xc1, 0x76, 0x94, 0xfa, 0xc6, 0xe9, 0x85, 0x27, 0xb6, 0x42, 0xf2, 0xed, 0xd5, 0xce, 0x61};
    var c_finished: [4 + Sha256.digest_length + 1]u8 = undefined;
    var c_finished_stream = io.fixedBufferStream(&c_finished);
    try c_finished_stream.writer().writeByte(@enumToInt(msg.HandshakeType.finished));
    try c_finished_stream.writer().writeIntBig(u24, Sha256.digest_length);
    try c_finished_stream.writer().writeAll(&c_hs_finished_digest);
    try expect(std.mem.eql(u8, c_finished[0..(try c_finished_stream.getPos())], &c_finished_ans));

    try c_finished_stream.writer().writeByte(0x16); // ContentType handshake

    const c_record_finished_ans = [_]u8{0x17, 0x03, 0x03, 0x00, 0x35, 0x75, 0xEC, 0x4D, 0xC2, 0x38, 0xCC, 0xE6, 0x0B, 0x29, 0x80, 0x44, 0xA7, 0x1E, 0x21, 0x9C, 0x56, 0xCC, 0x77, 0xB0, 0x51, 0x7F, 0xE9, 0xB9, 0x3C, 0x7A, 0x4B, 0xFC, 0x44, 0xD8, 0x7F, 0x38, 0xF8, 0x03, 0x38, 0xAC, 0x98, 0xFC, 0x46, 0xDE, 0xB3, 0x84, 0xBD, 0x1C, 0xAE, 0xAC, 0xAB, 0x68, 0x67, 0xD7, 0x26, 0xC4, 0x05, 0x46};
    var c_record_finished: [5 + c_finished.len + Aes128Gcm.tag_length]u8 = undefined;
    var c_record_finished_stream = io.fixedBufferStream(&c_record_finished);
    try c_record_finished_stream.writer().writeByte(0x17);
    try c_record_finished_stream.writer().writeIntBig(u16, 0x0303);
    try c_record_finished_stream.writer().writeIntBig(u16, c_record_finished.len - 5);
    Aes128GcmTLS.encrypt(&c_record_finished, &c_finished, hs_write_nonce, hs_write_key);
    try expect(std.mem.eql(u8, &c_record_finished, &c_record_finished_ans));

    const ap_write_key_ans = [_]u8{0x17, 0x42, 0x2d, 0xda, 0x59, 0x6e, 0xd5, 0xd9, 0xac, 0xd8, 0x90, 0xe3, 0xc6, 0x3f, 0x50, 0x51};
    const ap_write_iv_ans = [_]u8{0x5b, 0x78, 0x92, 0x3d, 0xee, 0x08, 0x57, 0x90, 0x33, 0xe5, 0x23, 0xd9};
    const ap_write_key = try key.hkdfExpandLabel(Sha256, c_ap_traffic_secret, "key", "", Aes128Gcm.key_length);
    try expect(std.mem.eql(u8, &ap_write_key, &ap_write_key_ans));
    const ap_write_iv = try key.hkdfExpandLabel(Sha256, c_ap_traffic_secret, "iv", "", Aes128Gcm.nonce_length);
    try expect(std.mem.eql(u8, &ap_write_iv, &ap_write_iv_ans));
    var ap_write_nonce = [_]u8{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
    i = 0;
    while (i < ap_write_iv.len): (i += 1) {
        ap_write_nonce[i] = ap_write_iv[i] ^ ap_write_nonce[i];
    }

    const ap_read_key_ans = [_]u8{ 0x9f, 0x02, 0x28, 0x3b, 0x6c, 0x9c, 0x07, 0xef, 0xc2, 0x6b, 0xb9, 0xf2, 0xac, 0x92, 0xe3, 0x56 };
    const ap_read_iv_ans = [_]u8{ 0xcf, 0x78, 0x2b, 0x88, 0xdd, 0x83, 0x54, 0x9a, 0xad, 0xf1, 0xe9, 0x84 };
    const ap_read_key = try key.hkdfExpandLabel(Sha256, s_ap_traffic_secret, "key", "", Aes128Gcm.key_length);
    try expect(std.mem.eql(u8, &ap_read_key, &ap_read_key_ans));
    const ap_read_iv = try key.hkdfExpandLabel(Sha256, s_ap_traffic_secret, "iv", "", Aes128Gcm.nonce_length);
    try expect(std.mem.eql(u8, &ap_read_iv, &ap_read_iv_ans));
    var ap_read_nonce = [_]u8{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
    i = 0;
    while (i < ap_read_iv.len): (i += 1) {
        ap_read_nonce[i] = ap_read_iv[i] ^ ap_read_nonce[i];
    }

    // STATE = CONNECTED

    // app_hs_stream = ClientHello || ServerHello || Certificate || CertificateVerify || ServerFinished || ClientFinished
    //try app_hs_stream.writer().writeAll(c_finished[0..(c_finished.len-1)]);
    try app_hs_stream.writer().writeAll(&c_finished_ans);
    const res_master_secret_ans = [_]u8{0x7d, 0xf2, 0x35, 0xf2, 0x03, 0x1d, 0x2a, 0x05, 0x12, 0x87, 0xd0, 0x2b, 0x02, 0x41, 0xb0, 0xbf, 0xda, 0xf8, 0x6c, 0xc8, 0x56, 0x23, 0x1f, 0x2d, 0x5a, 0xba, 0x46, 0xc4, 0x34, 0xec, 0x19, 0x6c};
    const res_master_secret = try key.deriveSecret(Sha256, master_secret, "res master", &app_hs_bytes);
    try expect(std.mem.eql(u8, &res_master_secret, &res_master_secret_ans));

    // decode NewSessionTicket
    const s_ticket_enc = [_]u8{};
    var s_ticket_record:[s_ticket_enc.len - 5 - 16]u8 = undefined;
    try Aes128GcmTLS.decrypt(&s_ticket_record, &s_ticket_enc, ap_read_nonce, ap_read_key);

    // send application_data
    var c_app_data:[51]u8 = undefined;
    for (c_app_data) |*value, app_i| {
        value.* = @intCast(u8, app_i);
    }
    c_app_data[50] = 0x17;

    const c_app_record_ans = [_]u8{0x17, 0x03, 0x03, 0x00, 0x43, 0xA2, 0x3F, 0x70, 0x54, 0xB6, 0x2C, 0x94, 0xD0, 0xAF, 0xFA, 0xFE, 0x82, 0x28, 0xBA, 0x55, 0xCB, 0xEF, 0xAC, 0xEA, 0x42, 0xF9, 0x14, 0xAA, 0x66, 0xBC, 0xAB, 0x3F, 0x2B, 0x98, 0x19, 0xA8, 0xA5, 0xB4, 0x6B, 0x39, 0x5B, 0xD5, 0x4A, 0x9A, 0x20, 0x44, 0x1E, 0x2B, 0x62, 0x97, 0x4E, 0x1F, 0x5A, 0x62, 0x92, 0xA2, 0x97, 0x70, 0x14, 0xBD, 0x1E, 0x3D, 0xEA, 0xE6, 0x3A, 0xEE, 0xBB, 0x21, 0x69, 0x49, 0x15, 0xE4};
    var c_app_record: [5 + c_app_data.len + Aes128Gcm.tag_length]u8 = undefined;
    var c_app_record_stream = io.fixedBufferStream(&c_app_record);
    try c_app_record_stream.writer().writeByte(0x17);
    try c_app_record_stream.writer().writeIntBig(u16, 0x0303);
    try c_app_record_stream.writer().writeIntBig(u16, c_app_record.len - 5);
    Aes128GcmTLS.encrypt(&c_app_record, &c_app_data, ap_write_nonce, ap_write_key);
    try expect(std.mem.eql(u8, &c_app_record, &c_app_record_ans));

    // recv application_data
    ap_read_nonce = [_]u8{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1}; // increment nonce counter
    i = 0;
    while (i < ap_read_iv.len): (i += 1) {
        ap_read_nonce[i] = ap_read_iv[i] ^ ap_read_nonce[i];
    }
    const s_app_record_enc = [_]u8{0x17, 0x03, 0x03, 0x00, 0x43, 0x2E, 0x93, 0x7E, 0x11, 0xEF, 0x4A, 0xC7, 0x40, 0xE5, 0x38, 0xAD, 0x36, 0x00, 0x5F, 0xC4, 0xA4, 0x69, 0x32, 0xFC, 0x32, 0x25, 0xD0, 0x5F, 0x82, 0xAA, 0x1B, 0x36, 0xE3, 0x0E, 0xFA, 0xF9, 0x7D, 0x90, 0xE6, 0xDF, 0xFC, 0x60, 0x2D, 0xCB, 0x50, 0x1A, 0x59, 0xA8, 0xFC, 0xC4, 0x9C, 0x4B, 0xF2, 0xE5, 0xF0, 0xA2, 0x1C, 0x00, 0x47, 0xC2, 0xAB, 0xF3, 0x32, 0x54, 0x0D, 0xD0, 0x32, 0xE1, 0x67, 0xC2, 0x95, 0x5D};
    var s_app_record:[s_app_record_enc.len - 5 - Aes128Gcm.tag_length]u8 = undefined;
    try Aes128GcmTLS.decrypt(&s_app_record, &s_app_record_enc, ap_read_nonce, ap_read_key);

    // send alert
    ap_write_nonce = [_]u8{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1};
    i = 0;
    while (i < ap_write_iv.len): (i += 1) {
        ap_write_nonce[i] = ap_write_iv[i] ^ ap_write_nonce[i];
    }

    const c_alert_data = [_]u8{0x01, 0x00, 0x15}; // ContentType alert
    const c_alert_ans = [_]u8{0x17, 0x03, 0x03, 0x00, 0x13, 0xC9, 0x87, 0x27, 0x60, 0x65, 0x56, 0x66, 0xB7, 0x4D, 0x7F, 0xF1, 0x15, 0x3E, 0xFD, 0x6D, 0xB6, 0xD0, 0xB0, 0xE3};
    var c_alert: [5 + c_alert_data.len + Aes128Gcm.tag_length]u8 = undefined;
    var c_alert_stream = io.fixedBufferStream(&c_alert);
    try c_alert_stream.writer().writeByte(0x17);
    try c_alert_stream.writer().writeIntBig(u16, 0x0303);
    try c_alert_stream.writer().writeIntBig(u16, c_alert.len - 5);
    Aes128GcmTLS.encrypt(&c_alert, &c_alert_data, ap_write_nonce, ap_write_key);
    try expect(std.mem.eql(u8, &c_alert, &c_alert_ans));

    // recv alert
    ap_read_nonce = [_]u8{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2}; // increment nonce counter
    i = 0;
    while (i < ap_read_iv.len): (i += 1) {
        ap_read_nonce[i] = ap_read_iv[i] ^ ap_read_nonce[i];
    }
    const s_alert_enc = [_]u8{0x17, 0x03, 0x03, 0x00, 0x13, 0xB5, 0x8F, 0xD6, 0x71, 0x66, 0xEB, 0xF5, 0x99, 0xD2, 0x47, 0x20, 0xCF, 0xBE, 0x7E, 0xFA, 0x7A, 0x88, 0x64, 0xA9};
    var s_alert:[s_alert_enc.len - 5 - 16]u8 = undefined;
    try Aes128GcmTLS.decrypt(&s_alert, &s_alert_enc, ap_read_nonce, ap_read_key);

    // End of connection
}
