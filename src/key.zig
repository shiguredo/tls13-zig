...
pub const KeyScheduler = struct {
    ...
    pub fn init(hkdf: crypto.Hkdf, aead: crypto.Aead, s_key: []const u8, psk: []const u8, allocator: std.mem.Allocator) !Self {
        var res = Self{
            .allocator = allocator,
        ...
        try res.generateEarlySecrets(s_key, psk);
        return res;
    }
    ...
    fn generateEarlySecrets(self: *Self, shared_key: []const u8, psk: []const u8) !void {
...
test "KeyScheduler AES128GCM-SHA256" {
    ...
    var ks = try KeyScheduler.init(hkdf, aead, &dhe_shared_key, &([_]u8{0} ** 32), std.testing.allocator);
    defer ks.deinit();
    ...
    const sh = (try Handshake.decode(io.fixedBufferStream(&sh_bytes).reader(), std.testing.allocator, null)).server_hello;
    defer sh.deinit();
    ...
    const pt_misc = try Protector.decryptFromCipherBytes(&enc_data, hs_read_nonce.slice()[0..12].*, ks.secret.s_hs_key.slice()[0..16].*, std.testing.allocator);
