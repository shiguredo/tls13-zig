...
pub const TLSClient = struct {
    ...
    pub fn recv(self: *Self, b: []u8, reader: anytype) !usize {
        ...
        while (!ap_recv) {
            ...
            const plain_record = try self.protector.decrypt(recv_record, nonce.slice(), self.ks.secret.s_ap_key.slice(), self.allocator);
            defer plain_record.deinit();
            self.recv_count += 1;
...
test "client test with RFC8448" {
    ...
    const pt_misc = try protector.decryptFromCipherBytes(&enc_data, nonce.slice(), ks.secret.s_hs_key.slice(), std.testing.allocator);
    ...
    const pt_s_ticket = try protector.decryptFromCipherBytes(&s_ticket_enc, nonce.slice(), ks.secret.s_ap_key.slice(), std.testing.allocator);
    ...
    const pt_recv_ap = try protector.decryptFromCipherBytes(&s_app_record_enc, nonce.slice(), ks.secret.s_ap_key.slice(), std.testing.allocator);
    ...
    const pt_recv_alert = try protector.decryptFromCipherBytes(&s_alert_enc, nonce.slice(), ks.secret.s_ap_key.slice(), std.testing.allocator);
