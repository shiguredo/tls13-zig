...
test "client test with RFC8448" {
    ...
    var hs_msg: [10000]u8 = undefined;
    var hs_stream = io.fixedBufferStream(&hs_msg);
    _ = try hs_stream.write(&client_hello_bytes);
    _ = try handshake.encode(hs_stream.writer());
    ...
    const c_hs_traffic_secret = try key.deriveSecret(Sha256, handshake_secret, "c hs traffic", hs_stream.getWritten());
    ...
    const s_hs_traffic_secret = try key.deriveSecret(Sha256, handshake_secret, "s hs traffic", hs_stream.getWritten());
    ...
    const pt_misc = try Protector.decryptFromCipherBytes(&enc_data, hs_read_nonce, hs_read_key, std.testing.allocator);
    ...
    var app_hs_bytes: [client_hello_bytes.len + server_hello_bytes.len + (enc_data.len - 6 - Aes128Gcm.tag_length) + 4 + Sha256.digest_length]u8 = undefined;
    ...
    try app_hs_stream.writer().writeAll(pt_misc.content);
    ...
    const c_record_finished = try Protector.encrypt(c_finished, hs_write_nonce, hs_write_key, std.testing.allocator);
    ...
    const pt_s_ticket = try Protector.decryptFromCipherBytes(&s_ticket_enc, ap_read_nonce, ap_read_key, std.testing.allocator);
    ...
    const c_app_record = try Protector.encryptFromPlainBytes(&c_app_data, .application_data, ap_write_nonce, ap_write_key, std.testing.allocator);
    ...
    const pt_recv_ap = try Protector.decryptFromCipherBytes(&s_app_record_enc, ap_read_nonce, ap_read_key, std.testing.allocator);
    ...
    const pt_recv_alert = try Protector.decryptFromCipherBytes(&s_alert_enc, ap_read_nonce, ap_read_key, std.testing.allocator);
